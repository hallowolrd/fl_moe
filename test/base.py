...
class ExpertResidualBottleneckBlock(nn.Module):
    """
    Expert 内部的 residual bottleneck block。
    ...
    输入和输出维度保持一致，因此残差分支不需要 projection。
    所有可训练层均为 nn.Linear，便于现有 expert-level K-FAC
    方法逐层收集 Kronecker factors。
    """
    ...
    def __init__(
        ...
        if bottleneck_dim <= 0:
            raise ValueError("bottleneck_dim must be greater than 0.")
    ...
    def forward(self, features: Tensor) -> Tensor:
        if features.ndim != 2:
            raise ValueError(
                "Expert residual block input must have shape [N, D], "
                f"but received {tuple(features.shape)}."
            )

        if features.shape[1] != self.input_dim:
            raise ValueError(
                f"Expert residual block expected feature dimension "
                f"{self.input_dim}, but received {features.shape[1]}."
            )
...
class ClassificationExpert(nn.Module):
    """
    近似等参数量的双 residual bottleneck 分类专家。
    ...
    新结构:
        ResidualBottleneckBlock(input_dim -> bottleneck_dim -> input_dim)
        ResidualBottleneckBlock(input_dim -> bottleneck_dim -> input_dim)
        Linear(input_dim, num_classes)

    bottleneck_dim 自动选择，使新 Expert 的参数量尽可能接近
    原始 input_dim -> hidden_dim -> num_classes Expert。
    ...
    hidden_dim 仍保留为配置字段，但现在表示旧 Expert 的
    参数预算参考宽度，不再是新 Expert 中某一层的实际宽度。
    """
    ...
    def __init__(
        ...
        self.bottleneck_dim = self._resolve_parameter_matched_bottleneck_dim()

        self.block1 = ExpertResidualBottleneckBlock(
            input_dim=self.input_dim,
            bottleneck_dim=self.bottleneck_dim,
        )
        self.block2 = ExpertResidualBottleneckBlock(
            input_dim=self.input_dim,
            bottleneck_dim=self.bottleneck_dim,
        )
    ...
    def _parameter_count_for_bottleneck(
        self,
        bottleneck_dim: int,
    ) -> int:
        d = self.input_dim
    ...
    def _resolve_parameter_matched_bottleneck_dim(self) -> int:
        d = self.input_dim
        c = self.num_classes
        ...
        ideal = (
            self.legacy_parameter_budget - constant
        ) / float(coefficient)

        lower = max(1, int(math.floor(ideal)))
        upper = max(1, int(math.ceil(ideal)))
        ...
        return min(
            ...
            key=lambda b: (
                abs(
                    self._parameter_count_for_bottleneck(b)
                    - self.legacy_parameter_budget
                ),
                b,
            ),
    ...
    @property
    def actual_parameter_count(self) -> int:
    ...
    def forward(self, features: Tensor) -> Tensor:
        ...
        if features.shape[1] != self.input_dim:
            raise ValueError(
                f"Expert expected feature dimension {self.input_dim}, "
                f"but received {features.shape[1]}."
            )
